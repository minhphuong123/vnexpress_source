...
<div class="clear"></div>
	<ul>
       ...
       <?php
          $tin = tincungloai($idTin,$row_chitiet['idLT']);
          while($rows = mysqli_fetch_array($tin)){
       ?>
        
        <li>       
             <a href="#"><img src="upload/tintuc/<?php echo $rows['urlHinh']?>" alt="<?php echo $rows['TieuDe']?>"></a> <br />
 			 <a class="title" href="index.php?p=chitiettin&idTin=<?php echo $rows['idTin']?>"><?php echo $rows['TieuDe']?></a>
             <span class="no_wrap">   
        </li>
        <?php
          }
